...
--[[
	...
	runLoop = game:GetService("RunService").RenderStepped:Connect(function(deltaTime)
		...
		if Target and not Freethrow() and not WhohasTheBall() and not ClosestIB() then
			...
			char.Humanoid.WalkSpeed = Target.Character.Humanoid.WalkSpeed
			if Target.Character.Humanoid.WalkSpeed < 15 then char.Humanoid.WalkSpeed = 15 end
			...
			if WhohasTheBall() then
				...
				if CornerDistance > -1 then
					...
					if game:GetService("Players"):FindFirstChild("frzcl") then
						if char:FindFirstChild("BallConnect") then
							--if passonce then
							--	passonce = false
								local args5 = {
									[1] = "Pass",
									[2] = game:GetService("Players").frzcl.Character,
									[3] = "Overhead"
								}
