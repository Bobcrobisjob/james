...
for _, child in ipairs(game.ReplicatedStorage:GetDescendants()) do
	if child.Name:lower():find(("specter"):lower()) and child.ClassName == "Animation" then
		table.insert(notrealmove, child.AnimationId)
	end
end
...
local hounce = false
game.UserInputService.InputBegan:connect(function(input)
	if input.KeyCode == Enum.KeyCode.C or input.KeyCode == Enum.KeyCode.Two or input.KeyCode == Enum.KeyCode.One or input.KeyCode == Enum.KeyCode.Tab or input.KeyCode == Enum.KeyCode.X then
		hounce = true
	end
end)
					...
					if game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
						...
						if game.Players.LocalPlayer.Character:GetAttribute("CurrentState") ~= "Unconscious" and game.Players.LocalPlayer.Character:GetAttribute("CurrentState") ~= "TrueStunned" and game.Players.LocalPlayer.Character:GetAttribute("CurrentState") ~= "Dying" then
							...
							if hounce then
								...
								Target.HumanoidRootPart.Position.Y-0.21, Target.HumanoidRootPart.Position.Z + Target.HumanoidRootPart.Velocity.Z * 0.14)) }):Play()
								task.wait(0.15)
