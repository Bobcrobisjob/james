...
--[[
	...
	function calculateRelease(p)
		return -0.075957*p+67.4256
	end	
	...
	runLoop = game:GetService("RunService").RenderStepped:Connect(function(deltaTime)
		...
		if Target and not Freethrow() and not WhohasTheBall() and not ClosestIB() then
			...
			char.Humanoid.WalkSpeed = Target.Character.Humanoid.WalkSpeed * 0.7
			if HoopDistance > 100 then
				MoveTo(goal.Position)
			else
				if workspace.Balls:FindFirstChild("Ball") then
					MoveTo(Target.Character.HumanoidRootPart.Position + path*3.75)
				end
			end
			game.Players.LocalPlayer.Character.HumanoidRootPart.CustomPhysicalProperties = PhysicalProperties.new(0.775, 0.300000012, 0.5, 1, 1) 
			...
			if Target.Character.HumanoidRootPart.SS_Meter.Enabled == true and MyDistance < 9 then
				VirtualInputManager:SendKeyEvent(true, "Space", false, game)
				VirtualInputManager:SendKeyEvent(false, "Space", false, game)
			end

		else
			if not Freethrow() then
				local CornerDistance = (char.HumanoidRootPart.Position - corner).Magnitude

				if CornerDistance > 10 then
					...
					if char:FindFirstChild("BallConnect") then
						...
						if game.Players.LocalPlayer.Backpack.ActionValues.Power.Value >= calculateRelease(game.Stats.Network.ServerStatsItem["Data Ping"]:GetValue()) then
				...
				if Target then
					local HoopDistance = (goalitself.Position - Target.Character.HumanoidRootPart.Position).Magnitude
					local path = (XZ(goalitself.Position) - XZ(Target.Character.HumanoidRootPart.Position)).unit
					if HoopDistance < 40 then
						MoveTo(Target.Character.HumanoidRootPart.Position + path*3.75)
					else
						MoveTo(corner)
					end
		...
		if char.Humanoid.WalkSpeed < 14 then char.Humanoid.WalkSpeed = 14 end
