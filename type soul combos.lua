...
--[[
	...
	function ClosestIB()
		...
		for i,v in next, game.Workspace.TempFolder:GetChildren() do
			if v and v.Name == "IB" then
				if v:FindFirstChild("IBCount").TeamIndicator.ImageColor3 == game.Players.LocalPlayer.Team.TeamColor.Color then
	...
	runLoop = game:GetService("RunService").RenderStepped:Connect(function(deltaTime)
		...
		if Target and not Freethrow() and not WhohasTheBall() and not ClosestIB() then
			...
			char.Humanoid.WalkSpeed = (Target.Character.HumanoidRootPart.Velocity).Magnitude
			if (Target.Character.HumanoidRootPart.Velocity).Magnitude < 14 then char.Humanoid.WalkSpeed = 14 end
			if HoopDistance > 85 then
				MoveTo(goal.Position)
				char.Humanoid.WalkSpeed = 16
			else
				MoveTo(Target.Character.HumanoidRootPart.Position + path*3)
			end
			game.Players.LocalPlayer.Character.HumanoidRootPart.CustomPhysicalProperties = PhysicalProperties.new(1, 0.300000012, 0.5, 1, 1) 
			...
			if WhohasTheBall() then
				...
				if CornerDistance > -1 then
					...
					if char:FindFirstChild("BallConnect") then
						...
						if game.Players.LocalPlayer.Backpack.ActionValues.Power.Value > 63.62 then
